...
import { LogEntry, LogActionType, QuickEntryRow, TemporaryExpressRow, DailySummary, Profile, Vehicle } from './types';
...
const App: React.FC = () => {
  ...
  const [offlineActionQueue, setOfflineActionQueue] = useState<any[]>(() => {
    try {
      const q = localStorage.getItem('logcash_offline_queue');
  ...
  // Listener nativo do botão voltar do Android (Capacitor)
  useEffect(() => {
  ...
  // 1. Verifica Sessão e carrega dados
  useEffect(() => {
      ...
      .then(session => {
        ...
        if (!session) {
          ...
          Promise.all([
            ...
            if (v && v.length > 0) {
              setVehicles(v);
              const active = v.find(veh => veh.is_active) || v[0];
              setActiveVehicle(active);
              setVehicleName(active.model);
            }
          }).catch(console.error);
        }
      })
  ...
  useEffect(() => {
    ...
    const unsubscribe = supabaseService.subscribeToLogs((updatedLogs, supabaseError) => {
      ...
      if (supabaseError) {
        ...
        setLogs(prevLogs => {
          // Os logs retornados pelo servidor são a fonte absoluta da verdade.
          // Ignoramos os logs temporários para não gerar duplicação quando o servidor sincronizar.
          // Em um app de produção, para evitar piscar a tela, cruzaríamos os timestamps ou um id idempotente,
          // mas para resolver a duplicação severa rapidamente, confiamos totalmente no banco de dados.
          return updatedLogs;
        });
  ...
  const handleQuickEntryExport = async (rows: QuickEntryRow[]) => {
    ...
    try {
      ...
      const formattedRows: TemporaryExpressRow[] = rows.map(r => ({
        ...
        totalValue: r.entregues * (profile?.package_value ?? 2.50)
      }));
  ...
  const summaryRows = useMemo(() => {
    ...
    filteredLogs.forEach(log => {
      ...
      if (log.type === 'ENTRADA') dailyData[day].loaded += 1;
        ...
        dailyData[day].gains += (log.value || 0);
      }
  ...
  const handleRouteActivitySave = async (data: { entrada: number, saida: number, devolucao: number, customDate?: Date }) => {
    if (isExecuting) return;
    setIsExecuting(true);

    const previousLogs = [...logs];
    const timestamp = data.customDate ? data.customDate.toISOString() : new Date().toISOString();
    ...
    if (data.saida > 0) {
      for (let i = 0; i < data.saida; i++) {
        newOptimisticLogs.push({ id: `temp-route-out-${Date.now()}-${i}`, type: 'SAIDA', value: (profile?.package_value ?? 2.50), timestamp, user_id: session?.user?.id });
      }
    }
    if (data.devolucao > 0) {
  ...
  const handleDeleteLog = async (dateStr: string) => {
    ...
    try {
      ...
      const updatedLogs = logs.filter(log => {
        ...
        return logDate !== dateStr.toUpperCase();
      });
  ...
  const handleDeleteMultipleLogs = async (ids: string[], dateLabel: string) => {
    setIsExecuting(true);
    const previousLogs = [...logs];
    try {
      ...
      const validDbIds = ids.filter(id => !id.toString().startsWith('temp-'));
      if (validDbIds.length > 0) {
        await supabaseService.deleteLogsByIds(validDbIds);
      }
  ...
  const handleSaveEditRoute = async (newData: { loaded: number, delivered: number, acareacao: number }) => {
    ...
    try {
      ...
      for (let i = 0; i < newData.delivered; i++) {
        newEntries.push({ type: 'SAIDA', timestamp, value: (profile?.package_value ?? 2.50), isPaid: false, user_id: session?.user?.id });
      }
      ...
      switch (actionToExecute) {
        ...
        case 'DEVOLUCAO':
          ...
          const newLog: LogEntry = {
            ...
            timestamp,
            value: actionToExecute === 'SAIDA' ? (profile?.package_value ?? 2.50) : 0,
            isPaid: false,
  ...
  const executeBulkAction = async (type: 'ENTRADA' | 'SAIDA', quantity: number) => {
    ...
    try {
      ...
      const newEntriesOptimistic: LogEntry[] = Array.from({ length: quantity }).map((_, i) => ({
        ...
        timestamp,
        value: type === 'SAIDA' ? (profile?.package_value ?? 2.50) : 0,
        isPaid: false,
        user_id: session?.user?.id
      }));
  ...
  const counts = useMemo(() => {
    ...
    const diff = now.getDate() - day + (day === 0 ? -6 : 1); // Monday is the start
    ...
    const fallbackValue = profile?.package_value || 2.50;

    const pending = deliveredLogs.filter(l => !l.isPaid).reduce((sum, l) => sum + (l.value || fallbackValue), 0);
    const paidCurrentCycle = deliveredLogs.filter(l => l.isPaid && new Date(l.timestamp) >= startOfMonth).reduce((sum, l) => sum + (l.value || fallbackValue), 0);
    const prevMonthPaid = deliveredLogs.filter(l => l.isPaid && new Date(l.timestamp) >= startOfPrevMonth && new Date(l.timestamp) < startOfMonth).reduce((sum, l) => sum + (l.value || fallbackValue), 0);
    const totalCumulativePaid = deliveredLogs.filter(l => l.isPaid).reduce((sum, l) => sum + (l.value || fallbackValue), 0);
    const totalValue = deliveredLogs.reduce((sum, l) => sum + (l.value || fallbackValue), 0);
    ...
    return {
      ...
      todayValue: todayLogs.filter(l => l.type === 'SAIDA').reduce((sum, l) => sum + (l.value || fallbackValue), 0),
      weekValue: logs.filter(l => l.type === 'SAIDA' && new Date(l.timestamp) >= startOfWeek).reduce((sum, l) => sum + (l.value || fallbackValue), 0),
      yearValue: logs.filter(l => l.type === 'SAIDA' && new Date(l.timestamp) >= startOfYear).reduce((sum, l) => sum + (l.value || fallbackValue), 0),
      ...
      prevWeek: logs.filter(l => l.type === 'SAIDA' && new Date(l.timestamp) >= startOfPrevWeek && new Date(l.timestamp) < startOfWeek).length,
      prevYear: logs.filter(l => l.type === 'SAIDA' && new Date(l.timestamp) >= startOfPrevYear && new Date(l.timestamp) < startOfYear).length,
      wallet: {
        pending,
        paid: paidCurrentCycle,
        prevPaid: prevMonthPaid,
        cumulative: totalCumulativePaid
      },
      totalValue
    };
  }, [logs, profile?.package_value]);
  ...
  return (
    <div className="min-h-screen bg-black text-slate-200 selection:bg-emerald-500/30 flex flex-col overflow-x-hidden relative">
        ...
        {activeTab === 'dash' ? (
          <div className="animate-in fade-in duration-500">
            <div className="mb-2">
              <EliteDashboard
                ...
                logs={logs}
                counts={counts}
                valorPorPacote={profile?.package_value ?? 2.50}
          ...
          <div className="animate-in fade-in duration-500">
            <ElitePersonalData
              ...
              userName={profile?.full_name || userName}
              vehicleName={activeVehicle?.model || vehicleName}
              onBack={() => setActiveTab('profile')}
              onSave={(name, vName) => {
                setUserName(name);
          ...
          <div className="animate-in fade-in duration-500">
            <EliteVehicleRegistration
              ...
              onSave={(vehicleData) => {
                supabaseService.saveVehicle(vehicleData).then(v => {
                  setActiveVehicle(v);
                  setVehicles(prev => {
                    const existing = prev.findIndex(p => p.id === v.id);
                    if (existing >= 0) {
                      const newV = [...prev];
                      newV[existing] = v;
                      return newV;
          ...
          <div className="animate-in fade-in duration-500">
            <EliteSettings
              ...
              onUpdateProfile={async (updates) => {
                const p = await supabaseService.updateProfile(updates);
                setProfile(p);
              }}
              onBack={() => setActiveTab('profile')}
              onLogout={() => setConfirmingAction('LOGOUT')}
      ...
      {/* PAINEL DE AJUSTES */}
      {showSettings && (
        <div className="fixed inset-0 z-[1000] flex items-end md:items-center justify-center bg-black/90 backdrop-blur-md p-0 md:p-4">
      ...
      {/* Navegação Inferior Principal (com safe-area para Android) */}
      {(['dash', 'stats', 'route', 'profile', 'tax-invoice', 'personal-data', 'tax-data', 'settings', 'extrato', 'express-report'].includes(activeTab)) && (
        <div className="fixed bottom-0 inset-x-0 z-[600] p-4 pb-[max(env(safe-area-inset-bottom),1rem)] bg-gradient-to-t from-black via-black/80 to-transparent pointer-events-none">
