...
const DailyStatsTable: React.FC<DailyStatsTableProps> = ({ logs, onDeleteIds, valorPorPacote }) => {
  ...
  const dailyGroups = logs.reduce((acc: any, log) => {
    ...
    if (log.type === 'SAIDA') {
      acc[date].saida += 1;
      acc[date].total += (log.value || valorPorPacote);
    }
  ...
  const monthlyGroups = logs.reduce((acc: any, log) => {
    ...
    if (log.type === 'SAIDA') {
      acc[monthKey].saida += 1;
      acc[monthKey].total += (log.value || valorPorPacote);
    }
