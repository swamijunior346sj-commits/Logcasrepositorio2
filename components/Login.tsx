          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {error && (
              <div className="rounded-xl px-4 py-3 text-center text-[10px] font-bold uppercase tracking-wider"
                style={{
                  background: error.includes('criada') || error.includes('enviado') ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.08)',
                  border: error.includes('criada') || error.includes('enviado') ? '1px solid rgba(16,185,129,0.2)' : '1px solid rgba(239,68,68,0.15)',
                  color: error.includes('criada') || error.includes('enviado') ? '#34d399' : '#f87171',
                }}>
                {error}
              </div>
            )}
            ...
            {!isSignUp && (
              <div className="flex justify-between items-center px-1">
                ...
                <button 
                  ...
                  className="text-[10px] text-secondary-gray hover:text-primary-gold transition-colors font-bold uppercase tracking-wider"
                >
                  Esqueceu a senha?
                </button>
              </div>
