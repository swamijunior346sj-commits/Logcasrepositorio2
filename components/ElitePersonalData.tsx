...
interface ElitePersonalDataProps {
    ...
    vehicleName: string;
    onBack: () => void;
    onSave: (name: string, vehicle: string) => void;
    onAddVehicle?: () => void;
}

const ElitePersonalData: React.FC<ElitePersonalDataProps> = ({ profile, setProfile, activeVehicle, userName, vehicleName, onBack, onSave, onAddVehicle }) => {
    ...
    const handleSave = async () => {
        ...
        setSaveError('');
        try {
            const updatedProfile = await supabaseService.updateProfile({ full_name: name, email, phone });
            if (setProfile) setProfile(updatedProfile);
            setShowSaveSuccess(true);
        } catch (e: any) {
            console.error('Erro ao salvar perfil:', e);
            setSaveError(e?.message || 'Erro ao salvar. Verifique se as tabelas foram criadas no Supabase.');
        } finally {
            setIsSaving(false);
        }
    };
    ...
    return (
        <div className="flex justify-center items-start min-h-screen bg-pitch-black animate-in fade-in duration-700">
            ...
            <div className="relative flex min-h-screen w-full flex-col overflow-x-hidden max-w-[430px] bg-pitch-black shadow-2xl pb-12">
                    ...
                    <div className="pt-6">
                        ...
                        <div className="space-y-4">
                            <button 
                                ...
                                className="w-full text-left ultra-thin-border rounded-lg p-4 flex items-center justify-between bg-transparent active:bg-primary-gold/10 transition-colors"
                            >
                                <div className="flex items-center">
                                    ...
                                    <div className="text-left">
                                        <p className="text-[13px] font-bold text-white tracking-tight">{activeVehicle?.model || vehicle}</p>
                                        <p className="text-[9px] text-white/40 font-bold tracking-[0.15em] mt-0.5 uppercase">
                                            {activeVehicle?.plate || '---'} • {activeVehicle?.category || 'CARGA'}
                                        </p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="material-symbols-outlined text-primary-gold/50 text-[18px]">edit</span>
                                </div>
                            </button>
                ...
                {saveError && (
                    <div className="mx-6 mt-4 p-3 rounded-lg border border-red-500/30 bg-red-500/10">
                ...
                <div className="mt-12 px-6 pt-12 mb-8">
                    <button
                        onClick={handleSave}
                        disabled={isSaving}
                        className="w-full outline-gold-btn rounded-lg py-4.5 h-[56px] font-bold text-[12px] uppercase tracking-[0.3em] flex items-center justify-center disabled:opacity-50"
                    >
                        {isSaving ? (
                            <div className="size-5 border-2 border-primary-gold border-t-transparent rounded-full animate-spin"></div>
                        ) : (
                            'Salvar Alterações'
                        )}
                    </button>
            ...
            {showSaveSuccess && (
                <div className="fixed inset-0 z-[1000] glass-overlay-premium flex items-center justify-center px-8 animate-in fade-in duration-300">
