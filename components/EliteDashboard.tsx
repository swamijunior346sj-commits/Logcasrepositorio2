...
const EliteDashboard: React.FC<EliteDashboardProps> = ({ userName, avatarUrl, logs, counts, valorPorPacote, onNavigate }) => {
    ...
    const totalCumulative = counts.totalValue;
    ...
    const weeklyPct = calcPct(weeklyEarnings, counts.prevWeek * valorPorPacote);
    const monthlyPct = calcPct(monthlyEarnings, counts.wallet.prevPaid);
    const annualPct = calcPct(annualProjection, counts.prevYear * valorPorPacote);
    ...
    const calcChartData = () => {
        ...
        const labels = ['', '', '', '', '', '', ''];
        const daysStr = ['DOM', 'SEG', 'TER', 'QUA', 'QUI', 'SEX', 'SAB'];

        for (let i = 0; i < 7; i++) {
            const d = new Date(startOf7Days);
            d.setDate(d.getDate() + i);
            labels[i] = daysStr[d.getDay()];
        }
        
        logs.forEach(log => {
            const logDate = new Date(log.timestamp);
            if (log.type === 'SAIDA' && logDate >= startOf7Days && logDate <= now) {
                const diffTime = logDate.getTime() - startOf7Days.getTime();
                const diffDays = Math.floor(diffTime / (1000 * 60 * 60 * 24));
                if (diffDays >= 0 && diffDays < 7) {
                    data[diffDays] += valorPorPacote;
                }
            }
        });
        ...
        data.forEach((val, i) => {
            ...
            if (i === 0) {
                ...
                const prevY = 145 - (data[i - 1] / maxVal) * 115;
                const cx = (prevX + x) / 2;
                pathStr += `C${cx},${prevY} ${cx},${y} ${x},${y} `;
            }
        });

        return { pathStr, fillPathStr: `${pathStr} V160 H0 Z`, labels };
    };

    const { pathStr, fillPathStr, labels } = calcChartData();
            ...
            <header className="relative pt-12 px-8 flex justify-between items-center z-10">
                <div className="flex items-center gap-4">
                    <div className="size-11 rounded-full border border-primary-gold/20 p-[2px]">
                        <div className="bg-center bg-no-repeat aspect-square bg-cover rounded-full size-full filter contrast-125 grayscale brightness-90" style={{ backgroundImage: `url("${avatarUrl || 'https://lh3.googleusercontent.com/aida-public/AB6AXuAlP_6m46cIx9G_FjX3ku2YaZwMvrfuyj4hQcf8xv7fAjTuzgSadvH49RYNSfu0UCU-L5UZp5I2y0Sn4kft4mYucmwGhEPsIi0hGtD62kD_ZLmldrXSqt9j8I8DaHcJFN81eauwKQcGbwshg1YO9KUWsElYrK-IHKuGVFVOVxDYaSI7_83oI1N7UfOVpnysry8y5V0QFDicN1tywt_1WP2IxuPM0ev4dx7JYogKpeaAYsLMSJjGbrfGwMfv_r50U4uzFZ7Zxe3KjKLJ'}")` }}></div>
                    </div>
                ...
                <section className="relative px-0 mb-8 z-10">
                    <div className="h-44 w-full relative">
                        <svg className="w-full h-full" preserveAspectRatio="none" viewBox="0 0 430 160">
                            ...
                            <path className="gold-gradient-fill" d={fillPathStr}></path>
                            <path className="animated-path" d={pathStr} fill="none" stroke="#EBC051" strokeLinecap="round" strokeWidth="1.5"></path>
                            {Array.from({length: 5}).map((_, i) => (
                                <line key={i} stroke="white" strokeOpacity="0.03" strokeWidth="0.5" x1={(430/6) * (i+1)} x2={(430/6) * (i+1)} y1="0" y2="160"></line>
                            ))}
                        </svg>
                    </div>
                    <div className="flex justify-between px-2 mt-3">
                        {labels.map((day, i) => (
                            <span key={i} className="text-[9px] font-medium text-white/20 tracking-widest uppercase">{day}</span>
                        ))}
                    </div>
                </section>
                ...
                <section className="px-8 space-y-4 pb-32 z-10 w-full">
                    <div className="glass-card rounded-2xl p-6 flex justify-between items-center text-left">
                        ...
                        <div className="text-right">
                            <span className={`text-[11px] block font-medium tracking-wider mb-1 ${weeklyPct.startsWith('-') ? 'text-red-400' : 'text-primary-gold'}`}>{weeklyPct}</span>
                    ...
                    <div className="glass-card rounded-2xl p-6 flex justify-between items-center text-left">
                        ...
                        <div className="text-right">
                            <span className={`text-[11px] block font-medium tracking-wider mb-1 ${monthlyPct.startsWith('-') ? 'text-red-400' : 'text-primary-gold'}`}>{monthlyPct}</span>
                            <div className="h-[1px] w-6 bg-primary-gold/30 ml-auto"></div>
                        </div>
                    </div>
                    ...
                    <div className="glass-card rounded-2xl p-6 flex justify-between items-center text-left">
                        ...
                        <div className="text-right">
                            <span className={`text-[11px] block font-medium tracking-wider mb-1 ${annualPct.startsWith('-') ? 'text-red-400' : 'text-primary-gold'}`}>{annualPct}</span>
                            <div className="h-[1px] w-6 bg-primary-gold/30 ml-auto"></div>
                        </div>
                    </div>
                </section>
