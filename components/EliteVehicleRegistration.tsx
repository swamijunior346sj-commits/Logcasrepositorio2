...
const EliteVehicleRegistration: React.FC<EliteVehicleRegistrationProps> = ({ vehicle, onBack, onSave }) => {
    const [model, setModel] = useState(vehicle?.model || 'Sprinter Executiva');
    const [plate, setPlate] = useState(vehicle?.plate || 'LUX-2024');
    const [color, setColor] = useState(vehicle?.color || 'Preto Metalizado');
    const [renavam, setRenavam] = useState(vehicle?.renavam || '12345678901');
    const [year, setYear] = useState(vehicle?.year || '2024/2025');
    const [capacity, setCapacity] = useState(vehicle?.capacity?.toString() || '1500');
    ...
    return (
        <div className="flex justify-center items-start min-h-screen animate-in fade-in duration-700">
            ...
            <div className="relative flex min-h-screen w-full flex-col overflow-x-hidden max-w-[430px] bg-pitch-black shadow-2xl">
                ...
                {/* MODAL SUCCESS */}
                {showSuccess && (
                    <div className="fixed inset-0 z-[1000] glass-overlay flex items-center justify-center px-8">
