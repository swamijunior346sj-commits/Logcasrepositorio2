                    ...
                    <button
                        onClick={() => {
                            import('../services/pdfService').then(({ generateWeeklyPDF }) => {
                                const counts = { delivered: rows.reduce((acc, row) => acc + row.delivered, 0), todayEntrada: 0, todaySaida: 0, todayDevolucao: 0, todayValue: rows.reduce((acc, row) => acc + (row.gains || 0), 0) };
