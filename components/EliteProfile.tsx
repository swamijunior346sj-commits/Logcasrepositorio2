...
const EliteProfile: React.FC<EliteProfileProps> = ({ profile, setProfile, userName, onBack, onLogout, onTaxData, onPersonalData, onSettings, onReset, onTaxInvoice }) => {
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const updatedProfile = await supabaseService.updateProfile({ avatar_url: publicUrl });
            if (setProfile) setProfile(updatedProfile);
        } catch (error) {
            console.error('Error uploading avatar:', error);
            alert('Falha ao enviar imagem. Verifique se o bucket avatars foi criado no Supabase.');
        } finally {
            ...
            <div className="relative flex min-h-screen w-full flex-col overflow-x-hidden max-w-[430px] bg-pitch-black pb-12">
                ...
                <div className="flex flex-col items-center px-6 pt-4 mb-10">
                    <div className="relative mb-8">
                        ...
                        <div className="relative size-36 rounded-full p-[1px] bg-gradient-to-b from-[#F9E29C] to-[#AA771C] neon-border cursor-pointer" onClick={handleAvatarClick}>
                            <div className="size-full rounded-full border-[4px] border-pitch-black overflow-hidden bg-black relative">
                                <img alt={userName} className={`w-full h-full object-cover grayscale-[0.2] ${isUploading ? 'opacity-50' : ''}`} src={profile?.avatar_url || "https://lh3.googleusercontent.com/aida-public/AB6AXuA1nEhj2i0eHOa4yvLOI-Dx4CqWgy1r6IMkLx-3M027NbZ1cNThkwpalKZztGYj5sSGca61nITxbzLudi0yBlOvvon8q2t93Vi_4csBT3YVnKBlkDlQWuT97fHVb7KTqKSzv4MfJqd28EGi0Kx5N1b6MNVe8ICvDN8Iabft_hh3-ZH7UuV7gKsKNQwb8YJXhSdAnvVNE8AdxofmUcHl0uLYrxEd2Js3lgD7G_nbyTxUMFMYF_WJrIUCjQZdhItrPaGAaK4z3KLrgHLF"} />
                                {isUploading && (
                                    <div className="absolute inset-0 flex items-center justify-center">
                                        <div className="size-8 border-2 border-primary-gold border-t-transparent rounded-full animate-spin"></div>
                                    </div>
                                )}
                            </div>
