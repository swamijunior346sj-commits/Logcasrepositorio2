      ...
      setTimeout(async () => {
        try {
          ...
          if (elementToPrint) {
            ...
            const fileName = `Extrato_LogCash_${new Date().getTime()}.pdf`;
            ...
            if (Capacitor.isNativePlatform()) {
              ...
              try {
                ...
                console.error('Erro no Capacitor Share:', e);
  ...
  const routeDataKey = () => {
    logs.forEach(log => {
      ...
      if (log.type === 'ENTRADA') dailyData[key].loaded += 1;
        ...
        dailyData[key].gains += (log.value || 0);
      }
...
export const generateWeeklyPDF = (data: {
  ...
  if (data.rows && data.rows.length > 0) {
    ...
    gains = data.counts?.todayValue || 0;
  }
